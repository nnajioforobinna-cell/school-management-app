...
import { Receipt, Plus, Pencil, Trash2, Wand2, Printer, Search } from 'lucide-react'
...
import { PaymentDialog, type EditablePayment } from '@/pages/fees/PaymentDialog'
    ...
    <div>
      ...
      {payFor && payFor.invoiceId && (
        <PaymentDialog
          schoolId={schoolId}
          currency={currency}
          invoice={{ invoiceId: payFor.invoiceId, name: payFor.name, total: payFor.total, paid: payFor.paid }}
          editing={editingPayment}
          collectedSessionId={session?.id ?? null}
          collectedTermId={terms?.find((t) => t.is_current)?.id ?? termId}
          onClose={() => {
            setPayFor(null)
